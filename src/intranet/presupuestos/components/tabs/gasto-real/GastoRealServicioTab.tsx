...
function GastoRealServicioRow({ item, cotizacionId, incidencias }: RowProps) {
...
export function GastoRealServicioTab({ cotizacionId }: { cotizacionId: number }) {
      ...
      <Table>
        ...
        <TableBody>
          {isLoading ? (
            ...
            items.map((item) => (
              <GastoRealServicioRow key={item.ID} item={item} cotizacionId={cotizacionId} incidencias={incidencias} />
            ))
          )}
        </TableBody>
      </Table>
